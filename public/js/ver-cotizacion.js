...
function enteroEnLetras(n){n=Math.floor(Math.abs(Number(n)||0));if(n===0)return"CERO";const u=["","UNO","DOS","TRES","CUATRO","CINCO","SEIS","SIETE","OCHO","NUEVE","DIEZ","ONCE","DOCE","TRECE","CATORCE","QUINCE","DIECISÃ‰IS","DIECISIETE","DIECIOCHO","DIECINUEVE","VEINTE","VEINTIUNO","VEINTIDÃ“S","VEINTITRÃ‰S","VEINTICUATRO","VEINTICINCO","VEINTISÃ‰IS","VEINTISIETE","VEINTIOCHO","VEINTINUEVE"],d=["","","VEINTE","TREINTA","CUARENTA","CINCUENTA","SESENTA","SETENTA","OCHENTA","NOVENTA"],c=["","CIENTO","DOSCIENTOS","TRESCIENTOS","CUATROCIENTOS","QUINIENTOS","SEISCIENTOS","SETECIENTOS","OCHOCIENTOS","NOVECIENTOS"];const m100=x=>x<30?u[x]:`${d[Math.floor(x/10)]}${x%10?` Y ${u[x%10]}`:""}`;const m1000=x=>x===100?"CIEN":`${c[Math.floor(x/100)]}${x%100?`${x>=100?" ":""}${m100(x%100)}`:""}`.trim();if(n<1000)return m1000(n);if(n<1e6){const a=Math.floor(n/1000),r=n%1000;return`${a===1?"MIL":`${m1000(a)} MIL`}${r?` ${m1000(r)}`:""}`;}if(n<1e9){const a=Math.floor(n/1e6),r=n%1e6;return`${a===1?"UN MILLÃ“N":`${enteroEnLetras(a)} MILLONES`}${r?` ${enteroEnLetras(r)}`:""}`;}return String(n);}
function importeLetras(v){const n=Math.round((Number(v||0)+Number.EPSILON)*100)/100,e=Math.floor(n),c=Math.round((n-e)*100),m=cotizacion.currency==="DOLARES"?"DÃ“LARES":"SOLES";return`SON ${enteroEnLetras(e)} Y ${String(c).padStart(2,"0")}/100 ${m}`;}
