...
async function consultarDocumentoAutomatico(){
  ...
  }catch(e){console.warn("Consulta automática:",e.message);}finally{consultandoDocumento="";}
}
...
function validarCliente(){const d=$("documentoCliente").value.trim();if(d.length!==8&&d.length!==11){alert("El documento debe tener 8 dígitos para DNI o 11 para RUC.");$("documentoCliente").focus();return false;}if(!$("nombreCliente").value.trim()){alert("Ingrese el nombre o razón social.");$("nombreCliente").focus();return false;}const c=$("correoCliente").value.trim();if(c&&!c.includes("@")){alert("Ingrese un correo válido.");$("correoCliente").focus();return false;}return true;}
