...
// Cargar .env desde la raíz; también admite backend/.env.
...
// Un ADMIN empresarial jamás puede crear SUPERADMIN.
  ...
  if (
    ...
    return {
      ...
      code:
        "COMPANY_NOT_FOUND",
      message:
        "La empresa asociada a este usuario no está disponible."
    };
  }
  ...
  if (
    ...
    return {
      ...
      code:
        "COMPANY_SUSPENDED",
      message:
        "El acceso de esta empresa está suspendido. Contacta a Franco Systems."
    };
  }
    ...
    if (
      ...
      return {
        ...
        code:
          "LICENSE_EXPIRED",
        message:
          "La licencia de Franco Systems ha vencido. Contacta al administrador para renovarla."
      };
    }
  }
  ...
  try {
    ...
    const {
      ...
      await admin
        .from("profiles")
        .select(
          "id, company_id, full_name, role, active"
        )
        .eq(
          "id",
          userData.user.id
    ...
    // SUPERADMIN no depende
    // de una licencia empresarial.
    if (
      profile.role !==
      "SUPERADMIN"
    ) {
    ...
    try {
      ...
      const {
        ...
        await admin
          .from("profiles")
          .select(
            "full_name, role, active, company_id"
          )
          .eq(
            "id",
            data.user.id
      ...
      // SUPERADMIN puede entrar siempre
      // mientras su usuario esté activo.
      if (
        profile.role !==
        "SUPERADMIN"
      ) {
  ...
  async (
    ...
    const {
      ...
      await admin
        .from("companies")
        .select(
          "id, name, ruc, plan, status, license_expires_at, max_users"
        )
        .eq(
          "id",
      ...
      return res
        ...
        .json({
          ...
          code:
            "API_TOKEN_REQUIRED",
          message:
            "Falta configurar API_TOKEN de Decolecta."
        });
    }
...
app.get(
  ...
  async (
    ...
    if (search) {
      ...
      query =
        query.or(
          `name.ilike.%${search}%,document.ilike.%${search}%`
        );
    }
...
app.post(
  ...
  async (
    ...
    if (
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "Documento y nombre son obligatorios."
        });
    }
    ...
    if (error) {
      ...
      const msg =
        error.code === "23505"
          ? "Ya existe un cliente con ese DNI o RUC."
          : error.message;
...
app.put(
  ...
  async (
    ...
    if (
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "El logo es demasiado pesado. Usa una imagen menor a 1 MB."
        });
    }
    ...
    const payload = {
      ...
      responsible_role:
        cleanText(
          req.body.responsible_role,
          250
        ),

      final_message:
        cleanText(
          req.body.final_message,
          1000
        ),
      ...
      bank_interbank:
        cleanText(
          req.body.bank_interbank,
          250
        ),

      bank_scotiabank:
        cleanText(
          req.body.bank_scotiabank,
          250
        ),
      ...
      return res
        ...
        .json({
          ...
          code:
            "USER_LIMIT_REACHED",
          message:
            `Esta empresa alcanzó su límite de ${maxUsers} usuario(s).`
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "Los planes PRUEBA, MENSUAL y ANUAL deben tener una fecha de vencimiento."
        });
    }
    ...
    const [
      ...
      await Promise.all([
        ...
        admin
          .from("quotes")
          .select(
            "id,quote_number,quote_date,client_name,total_amount,status,currency"
          )
          .eq(
            "company_id",
            req.profile.company_id
  ...
  async (
    ...
    let query =
      admin
        .from("quotes")
        .select(
          "id,quote_number,quote_date,client_document,client_name,document_type,currency,price_mode,total_amount,status,created_at"
        )
        .eq(
          "company_id",
          req.profile.company_id
    ...
    if (search) {
      ...
      query =
        query.or(
          `quote_number.ilike.%${search}%,client_name.ilike.%${search}%,client_document.ilike.%${search}%`
        );
    }
    ...
    if (!clientName) {
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "Selecciona o ingresa un cliente."
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "Reactiva la cotización antes de editarla."
        });
    }
...
function pdfMoney(
  ...
  return `${symbol} ${money(n).toLocaleString(
    "es-PE",
    {
...
app.get(
  ...
  async (
    ...
    for (
      ...
      doc.text(
        "Franco Systems · Business ERP v1.0",
        right - 190,
        footerY + 9,
        {
          width:
...
app.listen(
  ...
  () => {
    ...
    console.log(
      "FRANCO SYSTEMS ERP MULTIEMPRESA v1.2.0"
    );
